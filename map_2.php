...
// Add this function after your existing initMap function

// Make sure initMap is in the global scope
window.initMap = function() {
    map = new google.maps.Map(document.getElementById('map'), {
        center: { lat: 47.497912, lng: 19.040235 }, // Budapest center
        zoom: 8,
        styles: [
            {
                featureType: "transit.station",
...
// Helper function to get cached or new geocoding result
async function getLocationName(position) {
...
async function processBatch(stops, bounds) {
    const batchMarkers = [];
    const batchInfoWindows = [];
    ...
    await Promise.all(stops.map(async (stop) => {
        ...
        // Create marker immediately without waiting for geocoding
        const marker = new google.maps.Marker({
            position: position,
            map: map,
            title: stop.id,
        ...
        const infoWindow = new google.maps.InfoWindow({
            content: `
                <div class="p-4">
                    <h3 class="font-bold text-lg mb-2">Loading location...</h3>
                    <p class="text-sm mb-2">Station ID: ${stop.id}</p>
                    <p class="text-sm mb-2">Frequency: ${stop.frequency || '0'} utasok/nap</p>
                    <p class="text-sm mb-2">Agency: ${stop.agency_ids === '12' ? 'MÁV' : stop.agency_ids}</p>
                    <p class="text-xs mt-2">
        ...
        // Add click listener that will load location name when needed
        marker.addListener('click', async () => {
            infoWindows.forEach(w => w.close());
            
            // Update info window content with location name only when clicked
        ...
        batchInfoWindows.push(infoWindow);
    }));

    return { markers: batchMarkers, infoWindows: batchInfoWindows };
}
...
async function fetchAndDisplayStops() {
    try {
        ...
        for (let i = 0; i < stops.length; i += BATCH_SIZE) {
            ...
            const { markers: batchMarkers, infoWindows: batchInfoWindows } = 
                await processBatch(batch, bounds);
            
            markers.push(...batchMarkers);
            infoWindows.push(...batchInfoWindows);
            
            // Update map bounds and info panel periodically
            if (i % (BATCH_SIZE * 2) === 0) {
                map.fitBounds(bounds);
                updateInfoPanel(`
                    <div class="bg-white p-4 rounded-lg shadow">
                        <h4 class="font-bold text-lg mb-3">Loading stops... (${i}/${stops.length})</h4>
                        <div class="progress-bar">
                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                <div class="bg-blue-600 h-2.5 rounded-full" 
                                     style="width: ${(i / stops.length * 100)}%">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        // Final update of the info panel
        updateInfoPanel(`
            <div class="bg-white p-4 rounded-lg shadow">
                ...
                <div class="max-h-96 overflow-y-auto">
                    ${stops.map(stop => `
                        <div class="mb-3 p-3 bg-gray-50 rounded hover:bg-gray-100 cursor-pointer"
                             onclick="focusStop(${stop.latitude}, ${stop.longitude})">
                            <p class="font-bold">${stop.id}</p>
                            <p class="text-sm text-gray-600">Agency: ${stop.agency_ids === '12' ? 'MÁV' : stop.agency_ids}</p>
        ...
        updateInfoPanel('Error loading stops data: ' + error.message);
    }
}
</script>
