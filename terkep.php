...
<!-- -----------------------------------------------------------------------------------------------------HEADER END-------------------------------------------------------------------------------------------------- -->
    ...
    <div id="complex-route-select" class="hidden mb-6">
        ...
        <select id="complex-route" onchange="displayLocalRoute(this.value)" class="w-full p-3 border-2 rounded-lg focus:ring-2 focus:ring-blue-500">
...
function clearMap() {
    ...
    function updateRouteOptions() {
        const select = document.getElementById("complex-route");
        select.innerHTML = `<option value="">Válassz egy járatot</option>`;
...
async function loadRouteData() {
    try {
        ...
        // Convert API response into a usable structure
        routesData = data.reduce((acc, stop) => {
            if (!acc[stop.option]) acc[stop.option] = [];
...
function displayLocalRoute(routeId) {
    ...
    // Draw route on map
    currentPolyline = L.polyline(coordinates, { color: '#FF0000', weight: 3 }).addTo(map);
